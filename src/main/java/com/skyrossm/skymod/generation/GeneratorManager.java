...
public class GeneratorManager implements IWorldGenerator
{
	...
	private void generateSurface(World world, Random random, int x, int z){
		if(ConfigHandler.silverSpawn){
			this.addOreSpawn(ModBlocks.oreSilver, world, random, x, z, 16, 16, 4 + random.nextInt(3), 10, 5, 64);
		}
		...
		if(ConfigHandler.cobaltSpawn){
			this.addOreSpawn(ModBlocks.oreCobalt, world, random, x, z, 16, 16, 4 + random.nextInt(2), 3, 5, 32);
		}
		if(ConfigHandler.titaniumSpawn){
			this.addOreSpawn(ModBlocks.oreTitanium, world, random, x, z, 16, 16, 3 + random.nextInt(3), 7, 5, 24);
		}
		this.addOreSpawn(ModBlocks.oreMagic, world, random, x, z, 16, 16, 2 + random.nextInt(4), 5, 5, 256);
	}
